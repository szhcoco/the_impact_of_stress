...
function draw(student, exam, xScale, yScale, xAxisGroup, yAxisGroup, g, width, height, margin) {
    ...
    // Area
    Promise.all([
        d3.csv(`TEMP_min_max_${exam}.csv`, d3.autoType),
        d3.csv(`dataset/${student}_processed/${exam}/TEMP.csv`, d3.autoType)
        ])
        .then(([areaData, personData]) => {
          xScale.domain(d3.extent(personData, d => +d.time_seconds));
          yScale.domain([0, d3.max(personData, d => +d.TEMP)]).nice();
        ...
        const y = d3.scaleLinear()
            .domain([0, d3.max(downsampledData, d => d.TEMP_max, d3.max(personData, d => d.TEMP))]).nice()
            .range([height, 0]);
