  ...
  try {
    ...
    if (userLine) {
      ...
      const mse = d3.mean(data, d => {
        const yUser = userSlope * d[xField] + userIntercept;
        const yTrue = slope * d[xField] + intercept;
        return (yUser - yTrue) ** 2;
      });

      const maxPossibleError = d3.variance(data.map(d => d[yField]));
      const accuracy = Math.max(0, Math.min(100, 100 * (1 - mse / maxPossibleError)));
...
async function createTimeSeriesPlot(containerId, student, test, metric) {
  ...
  const data = await generateTimeSeriesData(student, test, metric);
  const minMaxData = await generateMinMaxData(test, metric);
  ...
  const periods = [
    { name: 'pre-test', start: 0, end: 300 },
    { name: 'in-test', start: 300, end: 3300 },
    { name: 'post-test', start: 3300, end: 3600 }
  ];
  ...
  interestingMoments.forEach((moment, i) => {
    ...
    const annotationCircle = annotationGroup.selectAll('circle').filter((d, j) => j === i);
    annotationCircle
...
// Modal functions
async function openStudentModal(studentData, metric) {
  try {
    ...
    await createEnlargedChart(studentData, metric);
    await updateStudentStats(studentData, metric);
...
async function createEnlargedChart(studentData, metric) {
  ...
  const periods = [
    { name: 'pre-test', start: 0, end: 300, label: 'Pre-Test' },
    { name: 'in-test', start: 300, end: 3300, label: 'During Test' },
    { name: 'post-test', start: 3300, end: 3600, label: 'Post-Test' }
  ];
...
async function updateStudentStats(studentData, metric) {
  try {
    ...
    const recoveryRate = inTestAvg - postTestAvg;
...
function showSlide(slideIndex) {
  ...
  if (slideIndex >= 1 && slideIndex <= 4) {
    const metrics = ['HR', 'EDA', 'TEMP', 'ACC'];
    const metric = metrics[slideIndex - 1];
...
// Hardcoded grades data as specified
const gradesData = {
...
// Data generation functions with real data loading
...
async function generateMinMaxData(test, metric) {
  ...
  // Generate mock min/max data as fallback
